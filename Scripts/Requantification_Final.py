...
def calculateRequant(outdir, CombinedFile, SR_R1, SR_R2, tmpdir, junc_cutoff, nthreads, LR_R1, LR_R2):
    ...
    COUNTS_SR=calculate_Junc_Span(str(outdir)+"/FinalFasta.fa", str(outdir)+"/filtered_SR.sorted.bam", junc_cutoff, outdir, requantificationBP, "SR")
    ...
    COUNTS_LR=calculate_Junc_Span(str(outdir)+"/FinalFasta.fa", str(outdir)+"/filtered_LR.sorted.bam", junc_cutoff, outdir, requantificationBP, "LR")
    ...
    CombinedFile['JR_SR']=pd.Series(JUNC_READS, index=CombinedFile.index)
    CombinedFile['SP_SR']=pd.Series(SPAN_READS, index=CombinedFile.index)
    CombinedFile['JR_LR']=pd.Series(JUNC_READS_NOR, index=CombinedFile.index)
    CombinedFile['SP_LR']=pd.Series(SPAN_READS_NOR, index=CombinedFile.index)
    return CombinedFile
...
if __name__=='__main__':
    ...
    FinalCombinedFile_2.to_csv(args.output, sep="\t", index=False)
