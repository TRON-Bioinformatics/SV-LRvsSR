    ...
    for index,row in short.iterrows():
        ...
        if check1==None:
            ...
            if check2==None:
                ...
                linked[check2[1]][check2[2]][check2[3]].pop(check2[4],None)
                Predicted_by.append("Common")
        else:
            linked[check1[1]][check1[2]][check1[3]].pop(check1[4],None)
            Predicted_by.append("Common")
...
if __name__=='__main__':
    ...
    if (not args.shortreads or not args.linkedDels or not args.linkedLarge):
        ...
        comparedDF=compare_SR_LR(linked, SV_short, int(args.window))
